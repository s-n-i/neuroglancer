...
export function getDesiredMultiscaleMeshChunks(
  ...
  function handleChunk(lod: number, row: number, priorLodScale: number) {
    ...
    const gridX = octree[rowOffset],
      ...
      childBeginAndVirtual = octree[rowOffset + 3],
    ...
    if (typeof window !== "undefined") {
      // console.log("manifest", manifest, window);
      console.log({
        lod,
        size,
        chunkShape,
      ...
      console.log({ xLower, yLower, zLower, xUpper, yUpper, zUpper });
    }
    if (isAABBVisible(xLower, yLower, zLower, xUpper, yUpper, zUpper, clippingPlanes)) {
      if (typeof window !== "undefined") {
        console.log("   bounding box is visible");
      }
      ...
      if (typeof window !== "undefined") {
        console.log("   bounding box is not visible");
      }
    }
  }
...
export function validateOctree(octree: Uint32Array, allowDuplicateChildren: boolean = false) {
  ...
  function exploreNode(node: number) {
    ...
    const x = octree[node * 5],
      y = octree[node * 5 + 1],
      z = octree[node * 5 + 2],
      beginChild = (octree[node * 5 + 3] & 0x7fffffff) >>> 0,
      endChild = (octree[node * 5 + 4] & 0x7fffffff) >>> 0;
    if (
      beginChild < 0 ||
      endChild < 0 ||
      endChild < beginChild ||
